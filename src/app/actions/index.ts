...
export async function getGenre(
  ...
  let url = `https://api.themoviedb.org/3/discover/${type}?with_genres=${genre}&sort_by=popularity.desc&page=${page}`;
...
export async function getMovie(id: number) {
  ...
  const resVideos = await fetch(`https://api.themoviedb.org/3/movie/${id}/videos`, {
    method: "GET",
    headers: {
      accept: "application/json",
      Authorization:
  ...
  if (!res.ok || !resCredits.ok || !resVideos.ok) {
...
export async function getTV(id: number) {
  ...
  const resVideos = await fetch(`https://api.themoviedb.org/3/tv/${id}/videos`, {
    method: "GET",
    headers: {
      accept: "application/json",
      Authorization:
